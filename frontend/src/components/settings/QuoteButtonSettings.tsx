...
export const QuoteButtonSettings: React.FC<SettingsComponentProps> = ({ settings, onChange }) => {
  ...
  return (
    <BlockStack gap="400">
        ...
        <Box paddingBlockStart="200">
          <BlockStack gap="200">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              />
              <code style={{ background: '#f4f6f8', padding: '4px 8px', borderRadius: '4px' }}>
                {settings.buttonColor || '#008060'}
              </code>
            </div>
          </BlockStack>
        </Box>
      ...
      <BlockStack gap="400">
        ...
        <BlockStack gap="200">
          ...
          <Text as="p" variant="bodySm" tone="subdued">
            Choose where the Quote button should appear relative to your theme's native buttons.
          </Text>
        </BlockStack>

        <BlockStack gap="200">
          <Text as="h3" variant="bodyMd" fontWeight="semibold">
            Advanced Position (Fallback)
          </Text>
          <Select
      ...
      <BlockStack gap="200">
        <Text as="h3" variant="headingSm" fontWeight="semibold">
          Which products should display the quote button?
        </Text>
        <RadioButton
          label="All products"
            ...
            <BlockStack gap="200">
              <Button onClick={handleSelectProducts}>
                {(settings.selectedProducts || []).length > 0
                  ? `Selected ${settings.selectedProducts?.length} products`
                  : 'Browse products'}
              </Button>
              <Text as="p" variant="bodySm" tone="subdued">
                The quote button will only appear on these items.
              </Text>
            </BlockStack>
          </Box>
        )}
            ...
            <BlockStack gap="200">
              <Button onClick={handleSelectCollections}>
                {(settings.selectedCollections || []).length > 0
                  ? `Selected ${settings.selectedCollections?.length} collections`
                  : 'Browse collections'}
              </Button>
              <Text as="p" variant="bodySm" tone="subdued">
                The quote button will only appear on products within these collections.
              </Text>
            </BlockStack>
          </Box>
        )}
