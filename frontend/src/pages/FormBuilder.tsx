...
export const FormBuilder: React.FC = () => {
      ...
      <Page title="Form Builder">
        <Box paddingBlockEnd="800">
          <Banner tone="warning" title="Upgrade required">
            <p>
              The form builder is only available on professional plans. Please upgrade to customize
              your quote form.
            </p>
          </Banner>
        </Box>
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const activeData = active.data.current as
      | { type?: string; elementType?: string; elementLabel?: string }
      | undefined;
    ...
    if (activeId !== overId && activeData?.type !== 'sidebar') {
      ...
      const overStep = formState.steps.find(
    ...
    if (activeData?.type === 'sidebar') {
      ...
      if (elementType && elementLabel) {
        ...
        const isEssential =
          ['email', 'phone'].includes(elementType.toLowerCase()) ||
          ['email address', 'phone number'].includes(elementLabel.toLowerCase());

        if (isEssential) {
          const exists = formState.steps.some((s) =>
            s.fields.some(
              (f) => f.type === elementType || f.label.toLowerCase() === elementLabel.toLowerCase(),
            ),
          );
          if (exists) {
            ...
            {selectedTab <= 1 ? (
              <>
                ...
                <main className="flex-1 h-full overflow-y-auto bg-[#fafbfc] preview-scrollbar border-r border-gray-100">
                  {selectedTab === 0 ? (
                    <BuilderCanvas formState={formState} setFormState={setFormState} />
                  ) : (
                      ...
                      <Card>
                        <BlockStack gap="400">
                          ...
                          <Text variant="bodyMd" as="p" tone="subdued">
                            Control what product information is shown to customers in the quote
                            request form.
                          </Text>
